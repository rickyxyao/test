...
proc upload data=file.initial_sample_cc out=tmp.sample; run;
...
proc sql;
	...
	select a.permno, a.confcalldate, b.tradingdate, b.tradingdate_plus1, b.tradingdate_6, b.tradingdate_252
	from sample as a, tradingdates as b
	where a.confcalldate <= b.tradingdate <= intnx('weekday',a.confcalldate,+10);
	quit;

data sample1; set sample1; id = cat(permno,confcalldate);
proc sort data=sample1; by id tradingdate; run;
data sample1; set sample1; by id; if first.id; drop id; * choose the next available trading date on or after the confcalldate;

proc sort data=sample; by permno confcalldate; run;
proc sort data=sample1; by permno confcalldate; run;

data sample; merge sample sample1; by permno confcalldate;

**** SHARE TURNOVER and PRE ALPHA FOLLOWING LOUGHRAN AND MCDONALD 2011 ****;

proc sql;
	create table temp as
	select a.permno, a.confcalldate, b.date, b.vol, b.ret
...
proc sql;
	...
	select a.permno, a.confcalldate, b.shrout
	from sample as a, crsp.dsf as b
	where a.permno=b.permno and a.tradingdate = b.date and (ret ne . and ret ne .B and ret ne .C);
	quit;

proc sort data=temp; by permno confcalldate date; run;
...
proc sort data=shrout nodupkey; by permno confcalldate; run;
proc sort data=volume; by permno confcalldate; run;
...
data tmp.turnover;
	...
	keep permno confcalldate volume shrout turnover;
	run;
	quit;
...
proc sort data=temp; by permno confcalldate; run;
...
data tmp.pre_alpha; set tmp.famafrench; pre_alpha = Intercept; keep permno confcalldate pre_alpha;
...
proc sql;
	create table temp as
	select a.permno, a.confcalldate, b.date, b.ret
	from sample as a, crsp.dsf as b
...
data tmp.car01;
	...
	car01 = ret - vwretd;
	keep permno confcalldate car01;
	run;
	quit;

**** COMBINE ALL CRSP DATA ****;

data turnover; set tmp.turnover; keep permno confcalldate volume shrout turnover;
data pre_alpha; set tmp.pre_alpha; keep permno confcalldate pre_alpha;
data nasdaq; set tmp.nasdaq; keep permno nasdaq;
data car01; set tmp.car01; keep permno confcalldate car01;

proc sort data=turnover; by permno confcalldate; run;
proc sort data=pre_alpha; by permno confcalldate; run;
proc sort data=nasdaq; by permno; run;
proc sort data=car01; by permno confcalldate; run;

data crspdata; merge turnover pre_alpha; by permno confcalldate; run;
data crspdata; merge crspdata nasdaq; by permno; run;
data crspdata; merge crspdata car01; by permno confcalldate; run;
...
proc download data=crspdata out=file.crspdata_cc; run;
